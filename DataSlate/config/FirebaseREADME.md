...
This guide explains how to configure Firebase for DataSlate.
...
| `DataSlate/GM_test.html` | Helper GM test view. |
| `DataSlate/DataSlate_test.html` | Helper player test view. |
...
2. Enter an app nickname, for example `WnG Tools DataSlate`.
3. Register the app.
...
If your group does not use Firebase Authentication for DataSlate, a simple temporary demo rule may be broader than production should allow. Do not leave open rules in a real public deployment.
...
Helper files are useful for testing configuration:
...
1. Open `DataSlate/GM_test.html`.
2. Open `DataSlate/DataSlate_test.html`.
...
| Ping does not play. | Browser audio policy or missing local audio file. | Interact with the page and verify `DataSlate/assets/audios/ping/Ping.mp3`. |
