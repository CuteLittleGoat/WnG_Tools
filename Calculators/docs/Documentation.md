...
- `CharacterCreation.html` – arkusz tworzenia postaci z walidacjami i przełącznikiem języka.
...
- `CharacterCreation.html`: reguła `.species-max-table td:first-child` utrzymuje lewostronne wyrównanie pierwszej kolumny oraz `font-weight: 600`.
...
Logika obliczeń, nazwy plików i odnośniki techniczne pozostają oparte na istniejących nazwach modułów, na przykład `XPCalculator.html`.

Warstwa angielska używa skrótu **XP**.

## 1.2. `CharacterCreation.html` — Szybkość i tabela maksimów ras
...
### 2.1. Fonty
- Wszystkie strony (`index.html`, `XPCalculator.html`, `CharacterCreation.html`) korzystają z lokalnego stosu fontów, bez zewnętrznych CDN:
...
### 2.3. Instrukcje PDF
- `HowToUse/pl.pdf` oraz `HowToUse/en.pdf` – otwierane przyciskiem **Instrukcja / Manual** w `CharacterCreation.html`.
...
Plik stosowany przez `XPCalculator.html` i częściowo przez `CharacterCreation.html`.
...
W `CharacterCreation.html` skrypt dodaje klasę `attribute-high` dla wartości atrybutów > 8. Styl nie jest zdefiniowany w CSS – to rezerwa na przyszłe wyróżnienie.

## 5. `XPCalculator.html` – kalkulator XP
### 5.1. Struktura HTML
...
## 6. `CharacterCreation.html` – arkusz tworzenia postaci
### 6.1. Struktura HTML
...
Modal maksymalnych wartości atrybutów znajduje się w `CharacterCreation.html`.
...
`CharacterCreation.html` rozszerza `kalkulatorxp.css`:
...
1. Utwórz cztery pliki: `index.html`, `XPCalculator.html`, `CharacterCreation.html`, `kalkulatorxp.css`.
2. Skopiuj wszystkie zmienne CSS i style z `kalkulatorxp.css` oraz inline z `index.html` i `CharacterCreation.html`.
3. Dodaj logo `Skull.png` i folder `HowToUse` z plikami `pl.pdf` i `en.pdf`.
4. Upewnij się, że w `XPCalculator.html` i `CharacterCreation.html` znajduje się link do `kalkulatorxp.css` (fonty są lokalne, bez Google Fonts).
...
Ekran startowy zawiera ukryty przycisk `#secretButton`, który po aktywacji przełącza użytkownika do `CharacterCreation.html`.
...
Definicje maksymalnych wartości atrybutów ras znajdują się w `XPCalculator.html` w bloku `speciesMaxAttributes` i są prezentowane jako tabela referencyjna pod tabelami **Atrybuty** i **Umiejętności**.
...
- Funkcja zapisu/odczytu postaci w `CharacterCreation.html` wymaga poprawnej konfiguracji Firebase (`Calculators/config/firebase-config.js`) oraz aktywnej bazy Firestore.
...
- Przyciski `Strona Główna / Main Page` (w `XPCalculator.html` i `CharacterCreation.html`) należy w docelowym wdrożeniu aplikacji zweryfikować pod kątem poprawnego `href`/adresu docelowego.
## 15. Kopia modułu dla nowej grupy
- `CharacterCreation.html` zawiera komentarze `WAŻNE/IMPORTANT` przy:
...
- Każda grupa powinna używać własnego pliku `Calculators/config/firebase-config.js` i własnych reguł dostępu do danych.
